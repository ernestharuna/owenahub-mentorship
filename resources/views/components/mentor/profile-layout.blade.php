    ...
    <div class="container mt-2">
        <section class="my-4 d-flex align-item-center gap-3">
            ...
            <div style="width: 70px; max-width: 70px; min-height: 70px; height: 70px;">
                ...
                @elseif (Auth::user()->display_image &&Auth::user()->display_image->image_path )
                    <img src="{{ asset('storage/' . Auth::user()->display_image->image_path) }}" alt="..."
                        class="border animated-2 fadeIn d-block img-fluid h-100 w-100 rounded-5 object-fit-cover border-3 border-warning">
                @else
